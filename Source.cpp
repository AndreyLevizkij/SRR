...
void main() {
	...
	do {
		cout << "Введите размер кванта: ";
		cin >> tq;
	} while (tq < 1);
	do {
		cout << "Введите скорость повышения приоритета нового процесса: ";
		cin >> a;
	} while (a < 0);
	do {
		cout << "Введите скорость повышения приоритета выбранного процесса: ";
		cin >> b;
	} while (b < 0);
	...
	for (int i = 0; i < n; i++) {
		cout << "Введите время появления процесса " << i + 1 << ": ";
		cin >> p[i].arrival_time;
		cout << "Введите время выполнения процесса " << i + 1 << ": ";
		cin >> p[i].burst_time;
	...
	int completed = 0; // Количество выполненных процессов
	int max_priority = 0; // Максимальный приоритет
	int min_priority_accepted = 0; // Минимальный приоритет
	int idx = -1;
		...
		for (int i = 0; i < n; i++) {
			if (burst_remaining[i] > 0 && p[i].arrival_time <= current_time) {
				if (p[i].priority >= min_priority_accepted && p[i].is_accepted == false) {
					p[i].is_accepted = true; cout << p[i].priority;
					while (!q.empty()){ q.pop(); }
				}
			}
		}		
	...
	cout << endl;
	for (int i = 0; i < n; i++) {
		cout << "Общее время в системе процесса " << i + 1 << ": " << p[i].total_time << endl;
	}
}